print('Задача 5. Текстовый редактор')

# Мы продолжаем разрабатывать новый текстовый редактор,
# и в этот раз нам поручили написать для него код,
# который считает количество любой буквы 
# и любой цифры в тексте (а не только буквы Ы как раньше).
# 
# Напишите функцию count_letters,
# которая принимает на вход текст и подсчитывает,
# какое в нём количество цифр K и букв N.
# 
# Функция должна вывести на экран информацию
# о найденных буквах и цифрах в определенном формате.
# 
# Пример:
# Введите текст: 100 лет в обед
# Какую цифру ищем? 0
# Какую букву ищём? л
# 
# Количество цифр 0: 2
# Количество букв л: 1
def count_letters(text):
  text = text # нужно для необязательной проверки
  input_numeral = input("Какую цифру ищем? ") 
  input_letter = input("Какую букву ищём? ")
  letter = 0
  numeral = 0
  ######################################### необязательная проверка
  result = 0
  for number in input_numeral:
    result += 1
  for number in input_letter:
    result += 1
  if result > 2:
    print("Введите одну цифру и одну букву")
    count_letters(text)
  #########################################
  for ex in text:
    if ex == input_numeral:
      numeral += 1
    elif ex == input_letter:
      letter += 1
  print ("Количество цифр:", input_numeral , "=", numeral)
  print ("Количество букв:", input_letter , "=", letter)


text = input("Введите текст: ")
count_letters(text)