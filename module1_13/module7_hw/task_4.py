...
for number in range(30, 36):
  print("Сектор:", number)
  peoples = int(input("Людей в секторе: "))
  if peoples > 10:
    problems += 1
    print("Нарушение! Слишком много людей в секторе!")
  else:
    print("Всё спокойно.")
print("Колличество нарушений: ", problems)

#Можно переменную number втиснуть в переменную peoples?? тоесть peoples = int(input("Людей в секторе(\number): ")). тоесть надо в текст впихнуть переменую но чтобы это осталось текстом. а иначе это уже не будет расмотрено как int, 